...
class RebootTestCase(unittest.TestCase):
    ...
    def test_reboot_now(self, mock_check_output):
        unattended_upgrade.reboot_if_requested_and_needed()
        mock_check_output.assert_called_with(["/sbin/shutdown", "-r", "now"], stderr=subprocess.STDOUT)

    @patch("subprocess.check_output", return_value="some shutdown msg")
    def test_reboot_time(self, mock_check_output):
        apt_pkg.config.set(
            "Unattended-Upgrade::Automatic-Reboot-Time", "03:00")
        unattended_upgrade.reboot_if_requested_and_needed()
        mock_check_output.assert_called_with(["/sbin/shutdown", "-r", "03:00"], stderr=subprocess.STDOUT)

    @patch("subprocess.check_output", return_value="")
    def test_reboot_withoutusers(self, mock_check_output):
        ...
        unattended_upgrade.reboot_if_requested_and_needed()
        mock_check_output.assert_called_with(["/sbin/shutdown", "-r", "04:00"], stderr=subprocess.STDOUT)
