...
class ConffilePromptTestCase(TestBase):
    ...
    def setUp(self):
        ...
        with open(os.path.join(self.rootdir, "etc/configuration-file"), "w") as fp:
...
""")
    ...
    def test_will_not_prompt_on_moves(self):
        ...
        test_pkg = os.path.join(self.packagedir, "test-package-2_1.3_all.deb")
        self.assertFalse(conffile_prompt(test_pkg, prefix=self.rootdir),
                         "conffile prompt detection incorrect")
    ...
    def test_with_many_entries(self):
        ...
        # many entries
        test_pkg = os.path.join(self.packagedir, "conf-test-package-257-conffiles_1.deb")
        self.assertFalse(conffile_prompt(test_pkg, prefix=self.rootdir),
                         "conffile prompt detection incorrect")

    def test_will_not_prompt_because_of_conffile_removal(self):
        # no conffiles anymore in the pkg
        test_pkg = os.path.join(self.packagedir, "conf-test-package-no-conffiles-anymore_2.deb")
        self.assertFalse(conffile_prompt(test_pkg, prefix=self.rootdir),
                         "conffile prompt detection incorrect")

    def test_will_prompt_multiple(self):
        # multiple conffiles
        test_pkg = os.path.join(self.packagedir, "multiple-conffiles_2_all.deb")
        self.assertTrue(conffile_prompt(test_pkg, prefix=self.rootdir),
                        "conffile prompt detection incorrect")
    ...
    def test_will_prompt_for_new_conffile(self):
        ...
        # becomes a conffile
        test_pkg = os.path.join(self.packagedir, "conf-test-package-new-conffile_1.deb")
        self.assertTrue(conffile_prompt(test_pkg, prefix=self.rootdir),
                        "conffile prompt detection incorrect")
